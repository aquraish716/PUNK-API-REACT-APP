...
likedPress = () => this.setState({liked: !this.state.liked})
  render()  {
    ...
    return (
        <div style={{color: 'green', textAlign: 'center', border: 'dashed', borderColor: 'black'}}>
            <p style={{color: 'black', textAlign: 'center', }}>{this.props.info.name}</p>
            <p style={{color: 'green', textAlign: 'center'}}>{this.props.info.tagline}</p>
            <button onClick={this.likedPress} style={{backgroundColor:`${buttonColor}`, color: 'black'}}>{buttonMessage}</button>
        </div>
    )
}
}
